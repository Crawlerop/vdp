...
typedef enum HLSFlags {
    ...
    HLS_WRITE_DESCRIPTION = (1 << 15),
    HLS_ALWAYS_WRITE_KEY = (1 << 16),
} HLSFlags;
...
static int hls_mux_init(AVFormatContext *s, VariantStream *vs)
{
    ...
    if (hls->segment_type == SEGMENT_TYPE_FMP4) {
        ...
        av_dict_set(&options, "mpegts_flags", "-write_af", AV_DICT_DONT_OVERWRITE);
    }
    ...
    if (remaining_options) {
        av_log(s, AV_LOG_WARNING, "Some of the provided format options are not recognized\n");
    }
    avio_flush(oc->pb);
    return 0;
}
...
static int hls_window(AVFormatContext *s, int last, VariantStream *vs)
{
    ...
    AVDictionaryEntry *stream_desc = NULL;
    const char *playlist_stream_desc;
	stream_desc = av_dict_get(s->metadata, "description", NULL, 0);
	playlist_stream_desc = stream_desc ? stream_desc->value : "no desc";
    ...
    for (en = vs->segments; en; en = en->next) {
        ...
        } else if ((hls->flags & HLS_ALWAYS_WRITE_KEY) && en == vs->segments) {
        ...
        ret = ff_hls_write_file_entry(byterange_mode ? hls->m3u8_out : vs->out, en->discont, byterange_mode,
                                      ...
                                      en->filename, prog_date_time_p, en->keyframe_size, en->keyframe_pos, hls->flags & HLS_I_FRAMES_ONLY, (hls->flags & HLS_WRITE_DESCRIPTION), playlist_stream_desc);
        if (ret < 0) {
    ...
    if (vs->vtt_m3u8_name) {
        ...
        for (en = vs->segments; en; en = en->next) {
            ret = ff_hls_write_file_entry(hls->sub_m3u8_out, 0, byterange_mode,
                                          ...
                                          hls->baseurl, en->sub_filename, NULL, 0, 0, 0, (hls->flags & HLS_WRITE_DESCRIPTION), playlist_stream_desc);
            if (ret < 0) {
                av_log(s, AV_LOG_WARNING, "ff_hls_write_file_entry get error\n");
            }
        }
    ...
    if (vs->packets_written && can_split && av_compare_ts(pkt->pts - vs->start_pts, st->time_base,
        ...
        if (hls->flags & HLS_SINGLE_FILE) {
            ...
            if ((hls->max_seg_size > 0 && (vs->size >= hls->max_seg_size)) || !byterange_mode) {
                ...
                if (use_temp_file)
                    av_dict_set(&options, "resend_headers", "1", 0);
...
static const AVOption options[] = {
    ...
    {"write_desc", "write segment descriptions", 0, AV_OPT_TYPE_CONST, {.i64 = HLS_WRITE_DESCRIPTION }, 0, UINT_MAX,   E, "flags"},
    {"always_write_key", "write hls key even if the stream wasn't encrypted", 0, AV_OPT_TYPE_CONST, {.i64 = HLS_ALWAYS_WRITE_KEY }, 0, UINT_MAX,   E, "flags"},
