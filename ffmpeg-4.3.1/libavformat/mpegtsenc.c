...
#define MPEGTS_FLAG_WRITE_SDT       0x20
#define MPEGTS_FLAG_NOPCR           0x40
#define MPEGTS_FLAG_WRITE_AF        0x80
#define MPEGTS_FLAG_VERSION_ONLY_PMT 0x100
#define MPEGTS_FLAG_FORCE_ADAPTATION 0x200
#define MPEGTS_FLAG_REVERSE_PMT_STREAMS 0x400
#define MPEGTS_FLAG_PCR_PID_IS_NULL 0x800
#define MPEGTS_FLAG_WRITE_SDT_LAST 0x1000
#define MPEGTS_FLAG_ADAPTATION_PADDING 0x2000
    int flags;
    int copyts;
    int resend_headers;
    int id3_tag;
...
static void mpegts_write_section_adapt(MpegTSSection *s, uint8_t *buf, int len, int padding)
{
    unsigned int crc;
    unsigned char packet[TS_PACKET_SIZE];
    const unsigned char *buf_ptr;
                                 ...
                                 uint8_t *buf, int len, MpegTSWrite *ts)
{
    ...
    *q++ = tid;
    ...
    put16(&q, id);
    if ((ts->flags & MPEGTS_FLAG_VERSION_ONLY_PMT) && tid != PMT_TID) {
    ...
    *q++ = last_sec_num;
    ...
    if ((ts->flags & MPEGTS_FLAG_FORCE_ADAPTATION) && (ts->flags & MPEGTS_FLAG_ADAPTATION_PADDING)) {
        s->cc = 15;
    }

    if (ts->flags & MPEGTS_FLAG_FORCE_ADAPTATION) {
        mpegts_write_section_adapt(s, section, tot_len, 170-len);
    } else { 
...
#define DEFAULT_PROVIDER_NAME   "IPTV"
#define DEFAULT_SERVICE_NAME    "Stream"
...
static int mpegts_write_pmt(AVFormatContext *s, MpegTSService *service)
{
    ...
    int val, stream_type, i, err = 0, have_id3 =0;

    q = data;
    if (ts->flags & MPEGTS_FLAG_PCR_PID_IS_NULL) {
    ...
    if (ts->id3_tag && (ts->id3_type == 1 || ts->id3_type == 2)) {
        val = 0xf011 | (q - program_info_length_ptr - 2);
    } else {
    ...
    if (ts->id3_tag) {
        const char *tag = "ID3 ";
        if (ts->id3_type == 1 || ts->id3_type == 2) { /* WMS optional ID3 */
            *q++ = 0x25; /* metadata descriptor */
            *q++ = 0x0f;
            ...
            *q++ = 0x1F;          /* metadata_locator_record_flag|MPEG_carriage_flags|reserved */
            put16(&q, 0x0001);
        }
        ...
        if (ts->id3_type == 1) {
            ...
            *q++ = 0xff;        /* metadata format */
            putbuf(&q, tag, strlen(tag));
            *q++ = 0x00;            /* metadata service ID */
            *q++ = 0x0F;          /* metadata_locator_record_flag|MPEG_carriage_flags|reserved */
        }
    }
    ...
    for (i = (ts->flags & MPEGTS_FLAG_REVERSE_PMT_STREAMS) ? s->nb_streams-1 : 0; i < s->nb_streams; (ts->flags & MPEGTS_FLAG_REVERSE_PMT_STREAMS) ? i-- : i++) {
        AVStream *st = NULL;
    ...
    if (ts->id3_tag) {
        if ((ts->id3_type == 0 || ts->id3_type == 2) && !(have_id3)) {
            const char *tag = "ID3 ";
            /* Stream metadata */
            *q++ = 0x15; /* stream type */
            put16(&q, 0xe000 | ts->id3_tag);
...
static int mpegts_init(AVFormatContext *s)
{
    ...
    if (ts->flags & MPEGTS_FLAG_FORCE_ADAPTATION) 
        av_log(s, AV_LOG_VERBOSE, "adaptation forced\n");

    if (ts->flags & MPEGTS_FLAG_NOPCR) 
        av_log(s, AV_LOG_VERBOSE, "pcr disabled\n");

    if (ts->flags & MPEGTS_FLAG_WRITE_AF) 
        av_log(s, AV_LOG_VERBOSE, "ram field wroten\n");
...
static void retransmit_si_info(AVFormatContext *s, int force_pat, int force_sdt, int64_t pcr)
{
    ...
    if ((ts->flags & MPEGTS_FLAG_WRITE_SDT) && (((pcr != AV_NOPTS_VALUE && ts->last_sdt_ts == AV_NOPTS_VALUE) ||
        ...
        mpegts_write_sdt(s);
    } else if ((ts->flags & MPEGTS_FLAG_WRITE_SDT_LAST) && (((pcr != AV_NOPTS_VALUE && ts->last_sdt_ts == AV_NOPTS_VALUE) ||
        (pcr != AV_NOPTS_VALUE && pcr - ts->last_sdt_ts >= ts->sdt_period) ||
        force_sdt))
    ) {
        send_sdt = 1;
...
/* Write a single blank af transport stream packet */
static void mpegts_insert_blank_af_packet(AVFormatContext *s, AVStream *st)
{
    MpegTSWriteStream *ts_st = st->priv_data;
    uint8_t *q;
    ...
    *q++ = 0x30 | ts_st->cc;   /* Adaptation both */
    ...
    *q++ = 0x00;               
    ...
    memset(q, 0xFF, TS_PACKET_SIZE - 6);
                             ...
                             int64_t pts, int64_t dts, int key, int stream_id)
{
    ...
    if (ts->flags & MPEGTS_FLAG_REEMIT_PAT_PMT) {
        force_pat = 1;
        if ((ts->flags & MPEGTS_FLAG_WRITE_SDT) || (ts->flags & MPEGTS_FLAG_WRITE_SDT_LAST)) {
            force_sdt = 1;
        }
        av_log(s, AV_LOG_VERBOSE, "reemit pat pmt for hls\n");
        ts->flags &= ~MPEGTS_FLAG_REEMIT_PAT_PMT;
    }
    ...
    while (payload_size > 0) {
        ...
        if (ts->mux_rate > 1) {
            ...
            if (dts != AV_NOPTS_VALUE && (dts - pcr / 300) > delay) {
                ...
                if (!(ts->flags & MPEGTS_FLAG_NOPCR) && write_pcr)
        ...
        *q++      = ts_st->pid;
        ts_st->cc = ts_st->cc + 1 & 0xf;
        av_log(s, AV_LOG_TRACE, "Calculated CC: %d\n", ts_st->cc);
        *q++      = 0x10 | ts_st->cc; // payload indicator + CC
        if (ts_st->discontinuity) {
            av_log(s, AV_LOG_TRACE, "Sending Discontinuity\n");
        ...
        if (key && is_start && pts != AV_NOPTS_VALUE) {
            ...
            if (ts_st->pcr_period) {
                av_log(s, AV_LOG_TRACE, "Writing PCR at START. Max PCR delay: %ld, Current PCR: %ld\n", delay, pcr);
                write_pcr = 1;
            }

            if (ts->flags & MPEGTS_FLAG_WRITE_AF) {
                av_log(s, AV_LOG_TRACE, "Setting RAM access at start\n");
...
static void mpegts_write_flush(AVFormatContext *s)
{
    ...
    if ((ts->flags & MPEGTS_FLAG_FORCE_ADAPTATION) && (ts->flags & MPEGTS_FLAG_ADAPTATION_PADDING)) {
        for (i = 0; i < s->nb_streams; i++) {
            ...
            MpegTSWriteStream *ts_st = st->priv_data;
            while (ts_st->cc != 15) {
                mpegts_insert_blank_af_packet(s, st);
            }
        }
    }
...
static const AVOption options[] = {
    ...
    { "mpegts_flags", "MPEG-TS muxing flags", OFFSET(flags), AV_OPT_TYPE_FLAGS, { .i64 = MPEGTS_FLAG_WRITE_AF }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "resend_headers", "Reemit PAT/PMT before writing the next packet",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_REEMIT_PAT_PMT }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "write_sdt", "Allow the writing of the SDT",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_WRITE_SDT }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "write_sdt_last", "Allow the writing of the SDT, but after the PMT packet",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_WRITE_SDT_LAST }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "write_af", "Random Access indicator on start",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_WRITE_AF }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "version_only_pmt", "Made table versions apply only to PMT",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_VERSION_ONLY_PMT }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "force_adaptation", "Force adaptation field to be set for sections",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_FORCE_ADAPTATION }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "pmt_desc", "Allow use of the descending PMT order",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_REVERSE_PMT_STREAMS }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "write_af_padding", "Write adaptation padding at flush",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_ADAPTATION_PADDING }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "null_pcr_pid", "PCR pid is NULL",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_PCR_PID_IS_NULL }, 0, INT_MAX, ENC, "mpegts_flags" },
    { "no_pcr", "Disable the writing of the PCR",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_NOPCR }, 0, INT_MAX, ENC, "mpegts_flags" },
    ...
    { "id3_tag", "Adds a dummy ID3 metadata specified by PID. 0 = Disable the generation", OFFSET(id3_tag), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, LAST_OTHER_PID, ENC },
    { "id3_type", "Specifies which type of ID3 metadata is gonna be written", OFFSET(id3_type), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 2, ENC },
