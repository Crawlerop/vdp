...
static const AVOption options[] = {
    { "movflags", "MOV muxer flags", offsetof(MOVMuxContext, flags), AV_OPT_TYPE_FLAGS, {.i64 = FF_MOV_FLAG_FASTSTART}, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
...
static int mov_write_sv3d_tag(AVFormatContext *s, AVIOContext *pb, AVSphericalMapping *spherical_mapping)
{
    ...
    const char* metadata_source = s->flags & AVFMT_FLAG_BITEXACT ? "Lavf" : "SHVDP/1.0";
                              ...
                              AVFormatContext *s)
{
    ...
    if (!mov_write_string_metadata(s, pb, "\251too", "encoding_tool", 1)) {
        if (!(s->flags & AVFMT_FLAG_BITEXACT))
            mov_write_string_tag(pb, "\251too", "SHVDP/1.0", 0, 1);
    }
                                        ...
                                        int has_h264, int has_video, int write_minor)
{
    ...
    else if (mov->mode == MODE_MP4 && mov->flags & FF_MOV_FLAG_DASH) 
...
static int mov_write_trailer(AVFormatContext *s)
{
    ...
    if (!(mov->flags & FF_MOV_FLAG_FRAGMENT)) {
        ...
        if (mov->mdat_size + 8 <= UINT32_MAX) {
            ...
            /* mdat_pos - 8 => size + wide */
            /* 00 00 00 01 <- 64 Bit Flag */
            /* atom */
            /* 00 01 02 03 04 05 06 07 <- 64 Bit Length */

            avio_seek(pb, mov->mdat_pos - 8, SEEK_SET);
            /* special value: real atom size will be 64 bit value after
             * tag field */
            avio_wb32(pb, 1); /* 0x00000001 */
            ffio_wfourcc(pb, "mdat"); /* mdat */
            avio_wb64(pb, mov->mdat_size + 16); /* always 0x8 in size, 64 bit */
        }
