...
from socket import SOL_SOCKET, SO_ERROR, SHUT_RDWR
...
class SSLInfo():
    def __init__(self,conn):
        self.alpn = conn.selected_alpn_protocol()
        self.npn = conn.selected_npn_protocol()
        self.cipher = conn.cipher()
        self.cipher_shared = conn.shared_ciphers()
        self.version = conn.version()
        self.ctx = conn.context
        self._sslconn = conn

    def getSession(self):
        return self._sslconn.session
...
class Socket(object):
    ...
    async def sendfile(self, fd, offset=0, size=None):
        self._socket.setblocking(True) 
        try:
            while True:
                try:
                    return self._socket.sendfile(fd, offset, size)
                except WantWrite:
                    await _write_wait(self._fileno)
                except WantRead:   # pragma: no cover
                    await _read_wait(self._fileno)
...
class SocketStream(StreamBase):
    ...
    async def _read(self, maxbytes=-1):
        while True:
            try:
                data = await self._socket_recv(maxbytes if maxbytes > 0 else MAX_READ)
                return data
            except WantRead:
                await _read_wait(self._fileno)
            except WantWrite:        # pragma: no cover
    ...
    async def write(self, data):
        ...
        try:
            while view:
                try:
                    nbytes = await self._socket_send(view)
                    nwritten += nbytes
                    view = view[nbytes:]
                except WantWrite:
                    await _write_wait(self._fileno)
    ...
    async def close(self):
        ...
        if self._file:
            try:
                await self._file.shutdown(SHUT_RDWR)
            except:
                pass
            await _io_release(self.fileno)
